...
if(Meteor.isClient){
  ...
  Router.onBeforeAction(filters.canView, {except: ['atSignIn', 'atSignUp', 'atForgotPwd', 'atResetPwd', 'signOut', 'voteInvitation']});
  Router.onBeforeAction(filters.hasCompletedProfile);
  Router.onBeforeAction(filters.isLoggedIn, {only: ['post_submit', 'voteInvitation']});
...
PostsListController = FastRender.RouteController.extend({
  ...
  data: function () {
    ...
    if (!this._terms.category) {
      parameters.find["categories.name"] = {$ne: "Ported"};
    } else if (this._terms.category === "none") {
      delete parameters.find['categories.slug'];
...
Meteor.startup(function () {
  ...
  Router.map(function() {
   ...
   // Loading (for testing purposes)
    ...
    // Voting Invites
    this.route('votingInvites',{
      template: getTemplate('votingInvites'),
      waitOn: function() {
        return coreSubscriptions.subscribe('allVotingInvites');
      },
      data: function() {
        var limit = parseInt(this.params.limit) || 20,
            parameters = getUsersParameters(this.params.filterBy, this.params.sortBy, limit),
    ...
    this.route('voteInvitation', {
      template: getTemplate('voteInvitation'),
      path: '/voteInvitation/:_id',
      waitOn: function() {
        return coreSubscriptions.subscribe('voteInvitation', this.params._id);
      },
      data: function() {
        return VotingInvites.findOne({_id: this.params._id});
      },
    });
